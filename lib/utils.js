...
function createRequest(api, params, args) {
    let apiVersion = api._getApiField('version');
    let versionCheck = compareVersionNumbers(params.minVersion, apiVersion);
    ...
    let path = `${buildBaseUrl(protocol, host)}/${params.path}`;
    ...
    let searchParamObj = buildBaseSearchParams(api);
